...
// 베이직 1개월 카드결제 -> 6개월 화면으로 갈때 DB에서 카드 번호 가져오기 
$(".basic01-card-basic06-card-btn").click(function() {	
	setTimeout(function() {
		$.ajax({
			type: "post",
			url: "/cardShow", // 요청 어디로갈지 수정 
			data: $("#basic-form06").serialize(),  // form 이름만 바꿔주자
			...
			success: function(data){
				...
				if (data != "") {
					...
					$(".basic-card-num06").text("카드번호 **** **** **** " + result);
					$(".basic-card-num06").append("<button>카드변경</button>");					
					$(".basic-card-num06 > button").addClass("basic-card06-btn");
					$(".basic-card06-btn").attr("type", "button");
					$('.basic-card06-btn').css({"color": "rgb(248, 47, 98)", 
					"margin-left": "10px"});			
				} else if (data == "") { 
					modal12.classList.toggle('show13');
				}	
			},
...
$(document).on("click", ".basic-card06-btn", function(){
	$("#card_Number").val('');   
	$("#card_Expiry").val('');
	$("#card_Birth").val('');
